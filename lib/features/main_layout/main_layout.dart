...
import 'package:ecommerce_c13_friday/features/cart/presentation/bloc/cart_bloc.dart';
...
class _MainLayoutState extends State<MainLayout> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
      getIt<HomeBloc>()
        ..add(HomeGetCategoriesEvent())..add(HomeGetCartEvent()),
      child: BlocBuilder<HomeBloc, HomeState>(
        builder: (context, state) {
          return Scaffold(
            appBar: HomeScreenAppBar(numOfProd:state.cartModel?.numOfCartItems??0,),
            extendBody: false,
